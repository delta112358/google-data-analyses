...
# This scripts tells me all the dates and times
# when I have been somewhere specific.
# See below at USER INPUT, to change point of interest, and date boundaries
# Download your own Location History JSON file at
...
def dist_btw_two_points(p1, p2):
    ...
    return 2 * r * np.arcsin(np.sqrt(np.sin((phi2 - phi1) / 2)**2
                                     + np.cos(phi1) * np.cos(phi2)
                                     * np.sin((lba2 - lba1) / 2)**2))
...
# # Some dicts only contain basic info :
...
for i in range(n):
    ...
    if ('latitudeE7' in point) and ('longitudeE7' in point):
        positions[i] = np.array([float(point['latitudeE7']),
...
# print("Total number of points with activity:
#        %d  (%0.1f%%)"%(n_act,int(n_act/n*100)))
...
# Converter
def ts2datetime(x):
    x_datetime = datetime.datetime.utcfromtimestamp(int(x / 1e3))
    return x_datetime.strftime('%Y-%m-%d %H:%M:%S')


# Get time boundary index
# np.searchsorted is a fast way to find the first element that
# is larger than the threshold. 1 is for True
...
for i in range(close_points.size):
    # If the delta between timestamps is bigger than the group size, or
    # if it's the beginning or the end.
    condition = timestampMs[close_points[i]] - \
        timestampMs[close_points[prev]] > grpsMs
    if condition or i == 0 or i == close_points.size - 1:
        # If it's not the beginning and
        # there are at least 2 points to make a group.
        if i > 0 and i - prev > 2:
            ...
            if group_verbosity == 2:
                print("\n\tGroup of %d points: %d -> %d" %
                      (i - prev - 2,
                       close_points[prev + 1], close_points[i - 1]))
